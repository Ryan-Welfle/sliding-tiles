...
pygame.init()
mixer.init()
mixer.music.load('SuperFrog - World 4.ogg')# This function allows you to play music but noramlly this should be in the ogg open source format
#Music gotten from amiga remix
...
WINDOWWIDTH = 1000  #dimensions of window itself, which I increased
WINDOWHEIGHT = 450 #dimensions of window itself, which I decreased
...
def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, RESET_SURF, RESET_RECT, NEW_SURF, NEW_RECT, SOLVE_SURF, SOLVE_RECT, HARD_SURF, HARD_RECT, FIVE_SURF, FIVE_RECT, SOLV20_SURF, SOLV20_RECT, imagelist, MUSIC_SURF, MUSIC_RECT, switch, HARD2_SURF, HARD2_RECT, MSTOP_SURF, MSTOP_RECT, MSTART_SURF, MSTART_RECT #added HARD_SURF, HARD_RECT, FIVE_SURF, FIVE_RECT, SOLV20_SURF, SOLV20_RECT, IMAGE as global variables
    ...
    MSTOP_SURF, MSTOP_RECT = makeText('Stop Background Music',TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 655, WINDOWHEIGHT - 40)#added button to allow user to stop background music
    MSTART_SURF,MSTART_RECT = makeText('Play Background Music',TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 655, WINDOWHEIGHT - 70)#added button to allow user to start background music
    MUSIC_SURF, MUSIC_RECT = makeText('Image #2 Easy Game',TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 975, WINDOWHEIGHT - 227.5)#added button to initiate easy game with second image
    HARD2_SURF, HARD2_RECT = makeText('Image #2 Hard Game',TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 975, WINDOWHEIGHT - 152.5)#added button to initiate hard game with second image
    ...
    NEW_SURF,   NEW_RECT   = makeText('Image #1 Easy Game',TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 975, WINDOWHEIGHT - 265) # changed vertical location of menu items
    HARD_SURF,  HARD_RECT  = makeText('Image #1 Hard Game',TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 975, WINDOWHEIGHT - 190) # adding a harder mode, so this is the button for it
    ...
    while True:
        slideTo = None
        msg = 'Click tile, press arrow keys, or press the number keys 1-4 to slide.' # contains the message to show in the upper left corner.
        if mainBoard == SOLVEDBOARD:
            msg='I knew you could do it!' #changed message here to something funnier
            ...
            if event.type == MOUSEBUTTONUP:
                ...
                if (spotx, spoty) == (None, None):
                    ...
                    if RESET_RECT.collidepoint(event.pos):
                        ...
                        allMoves = []
                    elif MSTOP_RECT.collidepoint(event.pos):#'elif' statement initiatives when 'Stop Background Music' is pressed
                        mixer.music.stop() #function that stops loaded music from playing
                    elif MSTART_RECT.collidepoint(event.pos):#'elif' statement initiatives when 'Stop Background Music' is pressed
                        mixer.music.play(-1) #function that plays loaded music; the '-1' parameter makes it so that music is played on a loop
                    elif HARD2_RECT.collidepoint(event.pos):
                        switch = 1 #allows second image to be used as the puzzle image
                        mainBoard, solutionSeq = generateNewPuzzle(350) # clicked on Easy Game button
                        allMoves = [] 
                        ...
                        switch = 0 #allows first image to be used as the puzzle image
                        mainBoard, solutionSeq = generateNewPuzzle(100) # clicked on Easy Game button
                        ...
                        resetAnimation(mainBoard, solutionSeq[-20:] + allMoves)
                ...
                if event.key in (K_LEFT, K_a,K_4) and isValidMove(mainBoard, LEFT):#added the 4 key to slide the tile left
                    slideTo = LEFT
                elif event.key in (K_RIGHT, K_d,K_3) and isValidMove(mainBoard, RIGHT):#added the 3 key to slide the tile right
                    slideTo = RIGHT
                elif event.key in (K_UP, K_w,K_1) and isValidMove(mainBoard, UP):#added the 1 key to slide the tile up
                    slideTo = UP
                elif event.key in (K_DOWN, K_s,K_2) and isValidMove(mainBoard, DOWN):# added 2 key to slide the tile down
                    slideTo = DOWN

        if slideTo:
            slideAnimation(mainBoard, slideTo, 'Click tile,press arrow keys to slide or press the number keys 1-4 to slide.', 8) # show slide on screen #added additional instruction 
...
def drawBoard(board, message):
    ...
    DISPLAYSURF.blit(MSTOP_SURF, MSTOP_RECT) #this code displays the 'stop background music' button
    DISPLAYSURF.blit(MSTART_SURF, MSTART_RECT) #this code displays the 'start background music' button
    DISPLAYSURF.blit(MUSIC_SURF, MUSIC_RECT) # this code displays the 'image #3 easy game' button
    DISPLAYSURF.blit(HARD2_SURF, HARD2_RECT)# this code displays the 'image #2 hard game' button
...
def resetLastFive(board, allMoves): # new function for resetting only last five moves
